...
class HuffmanFileSerializer:
    ...
    def __init__(self, filename: str, sep: str=None):
        self.filename = filename        # Src file name
        if sep is not None:
            if len(sep) <= 1:
                raise ValueError("Length of sep has to be greater than 1.")
            self.SEPARATOR = sep.encode("utf-8")
    
    def compress_file(self, dest_filename=None):
        text = "".join([i for i in file_reader(file_name=self.filename)])
        self.encode_and_write(text, dest_filename)

    def encode_and_write(self, text, dest_filename):
        ...
        padded_bytes_encoded_str = bytes(
            int("".join(padded_encoded_list[i:i+8]), 2)
            for i in range(0, len(padded_encoded_list), 8)
        )
        ...
        print(f"Encoded data written to {dest_filename}")

    @classmethod
    def decode_compress_file(cls, src_filename: str=None) -> str:
